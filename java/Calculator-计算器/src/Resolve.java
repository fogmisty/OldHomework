public class Resolve {
	...
	/**
	 * 中缀表达式转后缀表达式  
	 * 后缀表达式可以直接计算出结果，收藏里有  栈 优先级
	 * @param IFX
	 * @return
	 */
	public static MyQueue<String> TransIntoSuffix(String Infix)// IFX为中缀表达式 中缀表达式==最直观的式子，没啥变化
	{
		MyQueue<String> Suffix = new MyQueue<String>();// 队列
		StringBuffer numBuffer = new StringBuffer();// 用来保存一个数的
		MyStack<String> operater = new MyStack<String>();// 放操作符 栈
		String a;
		operater.push("=");// 第一个为等号
		int i = 0;
		char ch;
		for (i = 0; i < Infix.length();) {// 各种优先级问题
			ch = Infix.charAt(i);
			switch (ch) {
			case '0':
			case '1':
	 ...
	 * @return
	 */
	public static Double AngleFigure(MyQueue<String> Suffix) {// *180
		Double x1, x2, n;
		String str;
