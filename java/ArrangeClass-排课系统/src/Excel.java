...
public class Excel 
{
	...
	public Row getRow(int idxSheet,int idxRow) {
		return book.getSheetAt(idxSheet).getRow(idxRow);
	}
	public boolean load(String filename) throws IOException
	{
		...
		if(xlspos < filename.length() && xlspos+3 < filename.length()-1 && xlspos != -1) {
			book = new HSSFWorkbook(readFile);
			readFile.close();
			return true;
		}
		else if(xlsxpos < filename.length() && xlsxpos != -1) {
