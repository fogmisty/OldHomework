...
import com.example.game.gamehit.anima.UpDownAnima;
import com.example.game.gamehit.common.Const;
import com.example.game.gamehit.util.BitmapUtil;
import com.example.game.gamehit.util.GameUtil;
...
public class GameView extends View implements Runnable{
	...
	private Bitmap imgApple;
	private Bitmap imgWatermelon;
	private Bitmap imgPutao;
	private Bitmap imgBanana;
	private Bitmap imgHit;
	private Bitmap imgGround;
		...
		if(Const.gameMode==Const.gameMode_Level){
			for(int row=0; row<rowSize; row++){
				for(int col=0; col<colSize; col++){
					if(groundArray[row][col]!=0){
						canvas.drawBitmap(imgGround, (int) (col*Const.CURRENT_BLOCK_WIDTH), (int) (row*Const.CURRENT_BLOCK_HEIGHT), null);
					}
				}
			}
		}
...
//		runAwayNum = 0;
	...
	private void genDishu(){
		groundArray = GameUtil.getInstansce(getContext()).loadMapArrayByLevel(1, Const.gameArrayStrResid, rowSize, colSize);
		int rowSize = groundArray.length;
		int colSize = groundArray[0].length;
		int randomDishu = 0;
		...
		for(int row=0; row<rowSize; row++){
			for(int col=0; col<colSize; col++){
				if(randomDishu== groundArray[row][col]){
//					System.out.println("============="+randomDishu);
					if(randomShow(row, col)){
						randShow = rand.nextDouble();
						if(randShow>0.8){
							uda = new UpDownAnima(imgApple, imgHit, imgGround, 100, 8, (int) (col*Const.CURRENT_BLOCK_WIDTH), (int) (row*Const.CURRENT_BLOCK_HEIGHT), true);
						} else if(randShow>0.6){
							uda = new UpDownAnima(imgWatermelon, imgHit, imgGround, 100, 8, (int) (col*Const.CURRENT_BLOCK_WIDTH), (int) (row*Const.CURRENT_BLOCK_HEIGHT), true);
						} else if(randShow>0.4){
							uda = new UpDownAnima(imgPutao, imgHit, imgGround, 100, 8, (int) (col*Const.CURRENT_BLOCK_WIDTH), (int) (row*Const.CURRENT_BLOCK_HEIGHT), true);
						} else if(randShow>0.2){
							uda = new UpDownAnima(imgBanana, imgHit, imgGround, 100, 8, (int) (col*Const.CURRENT_BLOCK_WIDTH), (int) (row*Const.CURRENT_BLOCK_HEIGHT), true);
						} else{
							uda = new UpDownAnima(imgMe, imgHit, imgGround, 100, 8, (int) (col*Const.CURRENT_BLOCK_WIDTH), (int) (row*Const.CURRENT_BLOCK_HEIGHT), false);
						}
						uda.setRow(row);
						uda.setCol(col);
						dishuCount++;
	...
	private void initBitmap(){
		...
		imgApple = BitmapUtil.getInstansce(getContext()).getImgByResId(R.drawable.apple);
		imgWatermelon = BitmapUtil.getInstansce(getContext()).getImgByResId(R.drawable.watermelon);
		imgPutao = BitmapUtil.getInstansce(getContext()).getImgByResId(R.drawable.putao);
		imgBanana = BitmapUtil.getInstansce(getContext()).getImgByResId(R.drawable.banana);
		imgHit = BitmapUtil.getInstansce(getContext()).getImgByResId(R.drawable.afterhit);
		imgGround = BitmapUtil.getInstansce(getContext()).getImgByResId(R.drawable.ground);
		...
		imgSmallDishu40 = BitmapUtil.getInstansce(getContext()).getImgByResId(R.drawable.smallapple);
		imgX = BitmapUtil.getInstansce(getContext()).getImgByResId(R.drawable.x);
		imgMuchui = BitmapUtil.getInstansce(getContext()).getImgByResId(R.drawable.dao);
		imgMe = BitmapUtil.getInstansce(getContext()).getImgByResId(R.drawable.boom);
